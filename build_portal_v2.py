...
async function checkChatHealth() {{
  try {{
    const r = await fetch('http://localhost:8001/chat/health');
    if (r.ok) {{
      const data = await r.json();
      if (data.status === 'ok') {{
        setChatStatus('connectat · ' + (data.model || ''), 'online');
      }} else {{
        setChatStatus('error', 'offline');
      }}
    }} else {{
      setChatStatus('HTTP ' + r.status, 'offline');
    }}
  }} catch (e) {{
    setChatStatus('no connectat', 'offline');
  }}
}}
